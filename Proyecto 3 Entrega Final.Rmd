...
datos$DEPREG <- dep_decod[as.character(datos$DEPREG)]
datos$DEPOCU <- dep_decod[as.character(datos$DEPOCU)]
datos$PUEHOM <- pue_decod[as.character(datos$PUEHOM)]
datos$PUEMUJ <- pue_decod[as.character(datos$PUEMUJ)]
datos$ESCHOM <- esc_decod[as.character(datos$ESCHOM)]
datos$ESCMUJ <- esc_decod[as.character(datos$ESCMUJ)]
datos$MUPREG <- mun_decod[as.character(datos$MUPREG)]
datos$MUPOCU <- mun_decod[as.character(datos$MUPOCU)]
datos$NACHOM <- nac_decod[as.character(datos$NACHOM)]
datos$NACMUJ <- nac_decod[as.character(datos$NACMUJ)]
datos$CIUOHOM <- ciuo_decod[as.character(datos$CIUOHOM)]
datos$CIUOMUJ <- ciuo_decod[as.character(datos$CIUOMUJ)]

sum(is.na(datos$AÑOOCU))
...
media_edadH <- mean(datos$EDADHOM[datos$EDADHOM != 999], na.rm = TRUE)
media_edadM <- mean(datos$EDADMUJ[datos$EDADMUJ != 999], na.rm = TRUE)
datos$EDADHOM[datos$EDADHOM == 999] <- media_edadH
datos$EDADMUJ[datos$EDADMUJ == 999] <- media_edadM
...
resultados <- lapply(categoricas_vars, function(var) {
  valores <- valores_menos_frecuentes(datos, var)
...
datos_numericos <- datos[, c("AÑOREG", "AÑOOCU", "EDADHOM", "EDADMUJ")]
