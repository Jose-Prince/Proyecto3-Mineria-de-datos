...
datos_categoricos <- datos[, c("DEPREG", "MUPREG", "DEPOCU", "MUPOCU", "PUEHOM", "PUEMUJ", "NACHOM", "NACMUJ", "ESCHOM", "ESCMUJ", "CIUOHOM", "CIUOMUJ", "DIAOCU", "MESOCU")]
...
```{r group analysis categoric}

new_datos$DEPREG <- dep_decod[as.character(new_datos$DEPREG)]
new_datos$DEPOCU <- dep_decod[as.character(new_datos$DEPOCU)]
new_datos$PUEHOM <- pue_decod[as.character(new_datos$PUEHOM)]
new_datos$PUEMUJ <- pue_decod[as.character(new_datos$PUEMUJ)]
new_datos$ESCHOM <- esc_decod[as.character(new_datos$ESCHOM)]
new_datos$ESCMUJ <- esc_decod[as.character(new_datos$ESCMUJ)]
new_datos$MUPREG <- mun_decod[as.character(new_datos$MUPREG)]
new_datos$MUPOCU <- mun_decod[as.character(new_datos$MUPOCU)]
new_datos$NACHOM <- nac_decod[as.character(new_datos$NACHOM)]
new_datos$NACMUJ <- nac_decod[as.character(new_datos$NACMUJ)]
new_datos$CIUOHOM <- ciuo_decod[as.character(new_datos$CIUOHOM)]
new_datos$CIUOMUJ <- ciuo_decod[as.character(new_datos$CIUOMUJ)]

new_datos$grupo <- as.factor(kmeans$cluster)

frecuencias_categoricas <- function(x) {
  if (is.factor(x)) {
    return(table(x))
  } else {
    return(NA)
...
frecuencias_categoricas_grupo <- aggregate(new_datos[, sapply(new_datos, is.factor)], 
                                           by = list(new_datos$grupo), FUN = frecuencias_categoricas)
print("Frecuencias de Variables Categóricas por Grupo:")
print(frecuencias_categoricas_grupo)
